...
class SpinalNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x2 = self.fc_spinal_layer2(
            torch.cat([x[:, self.half_in_size : 2 * self.half_in_size], x1], dim=1)
        )
        x3 = self.fc_spinal_layer3(torch.cat([x[:, 0 : self.half_in_size], x2], dim=1))
        x4 = self.fc_spinal_layer4(
            torch.cat([x[:, self.half_in_size : 2 * self.half_in_size], x3], dim=1)
        )
...
class ResNetSpinalFc(nn.Module):
    def __init__(self, num_classes):
        ...
        self.model.fc = SpinalNet(
            num_classes, half_in_size=half_in_size, layer_width=layer_width
        )
...
class ConvNext(nn.Module):
    def __init__(self, num_classes=18):
        ...
        )  # pretrained convnext_base model을 사용
        self.base_model.head.fc = nn.Linear(
            self.base_model.head.fc.in_features, num_classes, bias=True
        )  # 마지막 레이어를 수정합니다.
...
class ConvNext_22k(nn.Module):
    def __init__(self, num_classes=18):
        ...
        )  # pretrained convnextv2_base model을 사용
        self.base_model.head.fc = nn.Linear(
            self.base_model.head.fc.in_features, num_classes, bias=True
        )  # 마지막 레이어를 수정합니다.
...
class Resnet18(nn.Module):
    def __init__(self, num_classes=18):
        ...
        self.base_model = models.resnet18(
            pretrained=True
        )  # pretrained resnet18 model을 사용
        num_ftrs = self.base_model.fc.in_features
        self.base_model.fc = nn.Linear(
            num_ftrs, num_classes, bias=True
        )  # 마지막 레이어를 수정합니다.
...
class DenseNet201(nn.Module):
    def __init__(self, num_classes=18):
        ...
        self.densenet_model = timm.create_model(
            "densenet201", pretrained=True, num_classes=num_classes
        )
...
class EfficientNetb2(nn.Module):
    def __init__(self, num_classes=18):
        ...
        self.efficientnet_model = timm.create_model(
            "efficientnet_b2", pretrained=True, num_classes=num_classes, drop_rate=0.2
        )
...
class EfficientNetb3(nn.Module):
    def __init__(self, num_classes=18):
        ...
        self.efficientnet_model = timm.create_model(
            "efficientnet_b3", pretrained=True, num_classes=num_classes, drop_rate=0.2
        )
...
class EfficientNetb4(nn.Module):
    def __init__(self, num_classes=18):
        ...
        self.efficientnet_model = timm.create_model(
            "efficientnet_b4", pretrained=True, num_classes=num_classes, drop_rate=0.2
        )
...
class EfficientNetV2_M(nn.Module):
    def __init__(self, num_classes=18):
        ...
        self.efficientnet_model = timm.create_model(
            "efficientnetv2_m", pretrained=True, num_classes=num_classes
        )
...
class VitB(nn.Module):
    def __init__(self, num_classes=18):
        ...
        self.vit_model = timm.models.vit_base_patch16_224(
...
if __name__ == "__main__":
    print("파라미터 갯수 출력")
    print(
        "Resnet 101 Spinal FC : ",
        sum(
            [
                param.numel()
                for param in ResNetSpinalFc(18).parameters()
    ...
    print(
        ...
        sum(
            [
                param.numel()
                for param in ResNext101(18).parameters()
                if param.requires_grad
            ]
        ),
    )
    print(
        ...
        sum(
            [
                param.numel()
                for param in Resnet34(18).parameters()
                if param.requires_grad
            ]
        ),
    )
    print(
        ...
        sum(
            [
                param.numel()
                for param in MobileNetV2(18).parameters()
                if param.requires_grad
            ]
        ),
    )
